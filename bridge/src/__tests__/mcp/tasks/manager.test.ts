...
describe('TaskManager', () => {
  ...
  describe('updateProgress (Track S)', () => {
    it('writes progress on a running task', () => {
      const task = taskManager.create({ type: 'chat', input: {}, sessionId: 's1' });
      taskManager.updateStatus(task.id, 'running');
      expect(taskManager.updateProgress(task.id, 'querying NetSuite')).toBe(true);
    ...
    it('truncates progress strings to 500 chars', () => {
      const task = taskManager.create({ type: 'chat', input: {} });
      taskManager.updateStatus(task.id, 'running');
      const long = 'x'.repeat(800);
      expect(taskManager.updateProgress(task.id, long)).toBe(true);
      expect(taskManager.get(task.id)?.progress?.length).toBe(500);
    });

    it('returns false for unknown task_id', () => {
      expect(taskManager.updateProgress('nonexistent', 'whatever')).toBe(false);
    });

    it('returns false for completed (terminal) task', () => {
      const task = taskManager.create({ type: 'chat', input: {} });
      taskManager.updateStatus(task.id, 'completed', 'done');
      expect(taskManager.updateProgress(task.id, 'too late')).toBe(false);
      expect(taskManager.get(task.id)?.progress).toBeUndefined();
    });

    it('overwrites existing progress on subsequent calls', () => {
      const task = taskManager.create({ type: 'chat', input: {} });
      taskManager.updateStatus(task.id, 'running');
      taskManager.updateProgress(task.id, 'stage 1');
      taskManager.updateProgress(task.id, 'stage 2');
      expect(taskManager.get(task.id)?.progress).toBe('stage 2');
    });
  });
