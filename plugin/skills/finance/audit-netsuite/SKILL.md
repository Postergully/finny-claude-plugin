...
description: Audit-style review of NetSuite records — anomalies, unusual transactions, control gaps. Maps to open-string intent `audit_review`. User invocation: /finny:finance:audit-netsuite [scope]
argument-hint: "[entity] [period] [audit focus] (optional — decomposer will ask if missing)"
---

# /finny:finance:audit-netsuite
...
Finny's brain knowledge of ShareChat's NetSuite tenant.

## Routing

This command maps to the OPEN-STRING intent `audit_review`. NOT in the bridge
bless-list — passes through to Finny. Hand off to `intent-decomposer` with:
...
Audit reviews are typically slower than direct queries (Finny may pull from
