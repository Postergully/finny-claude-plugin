...
description: Owns the discover→AskUser→execute orchestration for any Finny question. Activates immediately after finny-usage selects finny_query. Internalizes (never displays) discovery envelopes; biases toward asking the user when scope is ambiguous; gates first-call invented intents. Triggers: finny question, NetSuite question, ShareChat data question, P&L, vendor balance, open bills, finance command, intent decomposition.
---

# intent-decomposer

This is the load-bearing skill of the Finny plugin. Its job is to drive
`finny_query` through its two phases (`discover`, then `execute`) deterministically,
without leaking discovery output to the user, and without silent-guessing on
ambiguous scope.

## When this skill activates

Right after `finny-usage` decides the answer is `finny_query` (the 90% tool).
Does NOT activate for `finny_report` (registered reports skip discover) or
`finny_task_status`. Does NOT activate for free-form `finny_query({question})`
...
- The example clarifying questions Finny suggests.

You **never**:
- Paste the discover narrative.
- Paraphrase it as "here's what Finny came back with."
- Quote the GL-account counts, anomaly flags, or any internal-reasoning-shaped content.
- Show "based on what Finny told me, I think we need…" preambles. **The preamble is the same failure mode dressed up.**
...
Finny's example clarifying questions are technical (NetSuite-y). Use them as
...
> "I'd like to ask Finny about `<intent_id>`. Proceed?"
...
User: "give me P&L"
  │
  ▼
[finny-usage decides: finny_query, intent='p&l_statement']
  │
  ▼
[intent-decomposer takes over]
  │
  ▼
1. Show user: "Pulling scope info from Finny (~30-60s)…"
   Call finny_query({phase: 'discover', intent: 'p&l_statement', user_question: 'give me P&L'})
  │
  ▼
...
3. For each variable, decide: do I have a high-confidence value from context?
  ...
  │
  ▼
4. Once all variables resolved, call finny_query({
     phase: 'execute',
...
>
> *"Here's what Finny returned for discovery. Variables Finny needs: entity,
> period, consolidated, env. Brain hints: 193 GL accounts mapped, March 2026
> ₹208 Cr anomaly. Finny suggests these clarifying questions: 1) Which
...
> User: "give me P&L"
>
> [Status: "Pulling scope info from Finny (~30-60s)…"]
>
...
>
> [User answers. Calls finny_query phase:execute with full scope. Returns the
...
> *"Based on what Finny told me, I think we need to know which entity and period.
> Let me ask: …"*

✅ **Right:** AskUser directly. No "based on what Finny told me" framing. The user
...
defer to `judging-output`'s `needs_input` handler — Finny will ask back via
`finny_continue`.
...
these rules to Finny's example clarifying questions:
...
4. **Don't include the example wording verbatim** — Finny's phrasing is for
...
before calling finny_query with intent X where X not in bless-list:
  if X not in session_approved_intents:
    AskUser("I'd like to ask Finny about '" + X + "'. Proceed? (yes/no)")
    if yes:
...
- `finny-usage` — picks the tool BEFORE this skill activates. If finny-usage
  picks `finny_report`, this skill does NOT run.
...
Every Finny call: `finny-usage` → `intent-decomposer` (this skill) → tool
